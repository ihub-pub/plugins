 ...
 */
@IHubPlugin(
    value = IHubDemoExtension::class,
    beforeApplyPlugins = [IHubSimplePlugin::class],
    tasks = [IHubDemoTask::class]
)
class IHubDemoPlugin : IHubProjectPluginAware<IHubDemoExtension>() {
    ...
    override fun apply() {
        ...
        withTask<DefaultTask>("demo") {
            logger.info(it.name)
        }
        withTask(DefaultTask::class.java) {
            logger.info(it.name)
        }
        withTask<DefaultTask>("demo")
    }
